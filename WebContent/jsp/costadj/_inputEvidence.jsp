	...
	<%
      ...
      if (id != null) {
         ...
         if (actor.equals("과제책임자")) // 과제책임자일때   
         {
            ...
            else if (pr.getStatus().equals("중간평가완료"))
               response.sendRedirect("task_manager_inputEvidence.jsp?select_project_number="
                     + pr.getProjectNumber()); // 사업비증빙자료 추가 바운더리로 이동
            else if(pr.getStatus().equals("등록승인") || pr.getStatus().equals("중간평가계획"))
            	response.sendRedirect("error_calculation.jsp?error_msg=CA008"); // 로그인을 요청하는 에러페이지
            else
