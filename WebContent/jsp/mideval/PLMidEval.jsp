...
<%@ page import="Common.MidEvalResult" %>
...
<% 
	...
	ArrayList<MidEvalResult> midResult = new ArrayList<MidEvalResult>();
	midResult = midManager.requestMidEvalResultForPl(loginId);
...
<div id="contents">
	<form method="post" name="chargerInput" action="/ResearchMasterProject/jsp/common/Main.jsp">
		...
		<table>
			...
			<tr>
				<td><%= midResult.get(i).getProjectNumber() %></td>
				<td><%= midResult.get(i).getProjectLeaderId() %></td>
				<td><%= midResult.get(i).getFileName() %></td>
				<td><%= midResult.get(i).getResult() %></td>
			</tr>
			<%} %>
			<tr>
