...
public class UserManage {
	...
	public boolean reqjoin(User user) // 회원가입(사용자 :사용자) :boolean
	{
		...
		UserPermission up = new UserPermission(user.getId(),"사용자");
		pm.addUserPermission(up);
	...
	public boolean reqlogin(String id, String password) // 로그인(아이디 String ,패스워드
														// String)
	{
	...
	public ArrayList<User> reqUserListFromName(String userName) // req사용자목록(이름
																// :String)
																// :ArrayList<User>
	{
		UserList userlist = UserList.getInstance();
	...
	public boolean reqchecksocialNumber(String socialNumber) // req주민등록번호확인(주빈등록번호:String)
																// : boolean
	{
		UserList userlist = UserList.getInstance();
	...
	public ArrayList<User> reqUserListFromOrganName(String organName) // req사용자목록(기관명:String):ArrayList<User>
	{
		...
		for (int i = 0; i < orgarr.size(); i++) {
			System.out.println(orgarr.get(i).getOrganName());
			if (orgarr.get(i).getOrganName().equals(organName))
				organid = orgarr.get(i).getOrganId();
		}

		for (int i = 0; i < userarr.size(); i++) {
			if (userarr.get(i).getOrganId() == organid) {
	...
	public boolean checkPermission(String loginid) //사용자권한확인(아이디:String):boolean
	{
		PermissionManager pm = new PermissionManager(); 
